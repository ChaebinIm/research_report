...
## 0.5 용도 먼저 이해하기 — 이 기판들은 어떤 기계에 들어가나

"스위치·라우터·가속기 보드"라는 말 자체가 낯설다면, AI 데이터센터를 **공장 단지**로 그려보자:

- **GPU 서버 = 일하는 공장**: AI 계산(챗GPT의 답 생성, 모델 학습)을 실제로 수행하는 기계. 안에 GPU라는 초고가 칩이 8개씩 들어 있다
- **네트워크 스위치 = 단지의 우체국(교차로)**: 공장이 수만 개인데, AI 학습은 **모든 공장이 쉴 새 없이 서로 편지(데이터)를 주고받아야** 돌아간다. 그 편지를 중계·배분하는 전용 기계가 스위치이고, 스위치들을 더 크게 묶는 게 라우터다. '800G'는 이 우체국이 **초당 800기가비트**(영화 수십 편 분량/초)를 처리한다는 등급
- **일반 서버 = 사무동**: 이메일·웹사이트·데이터 저장 같은 평범한 일을 하는 컴퓨터들

이제 세 기판이 각각 어느 기계의 어느 부위인지 보면:

**① 일반 MLB → '사무동 컴퓨터의 몸통 판'.** 서버·통신장비를 열면 나오는 큰 초록색 판(메인보드) — CPU·메모리·전원부 같은 부품이 꽂히고 서로 연결되는 바탕이다. 개인 PC 메인보드의 업그레이드판이라고 보면 되고, 요구 스펙이 상대적으로 평범해서 가격 경쟁 시장이다.

**② 멀티램 → '우체국(800G 스위치)의 몸통 판'.** 스위치를 열면 앞면에 광케이블 꽂는 구멍이 수십 개, 안쪽엔 편지 분류를 담당하는 대형 칩(스위치 ASIC)이 있다. 문제는 배선이다 — **구멍 수십 개 × 각각 초고속 신호선 여러 가닥이, 서로 닿거나 간섭하지 않고 칩까지 도달**해야 한다. 2차원 평면에선 절대 못 푸는 교통량이라 도로를 30층+로 쌓아야 하고(초고다층), 판도 커야 하며(대면적), 신호가 조금만 왜곡돼도 800G 속도가 깨지니 노이즈 제거(백드릴)까지 필요하다 — 멀티램이 필요한 이유가 전부 이 안에 있다. **AI 붐 = GPU만 느는 게 아니라 우체국이 같이 늘어야 하는 붐**이라, 이 시장이 폭증 중이다.

**③ HDI → 'GPU 칩이 앉는 방석 판'.** 공장(GPU 서버) 안에서 GPU 칩이 실제로 장착되는 보드다. GPU 한 개의 발바닥엔 전기·데이터 핀이 **수천 개** 달려 있는데, 그 핀들을 칩 바로 아래 손바닥만 한 면적에서 전부 받아 사방으로 흩어줘야 한다. 층수를 늘려서 해결이 안 되는, **좁은 면적에 배선을 얼마나 촘촘히 욱여넣느냐(밀도)**의 문제 — 그래서 지름길 비아(마이크로비아)를 쓰는 HDI가 필요하다.

**한 랙(서버 캐비닛)에서의 흐름으로 정리하면**: GPU가 **HDI 보드** 위에서 계산하고 → 그 결과가 **멀티램 보드**로 만든 스위치를 거쳐 다른 GPU들에 전달되고 → 저장·관리는 **일반 MLB** 서버가 맡는다. 즉 세 제품은 경쟁 관계가 아니라 **AI 데이터센터라는 한 몸의 서로 다른 장기**이고, 이수페타시스는 그중 돈이 되는 순서(스위치→가속기)로 올라타는 중이다.
